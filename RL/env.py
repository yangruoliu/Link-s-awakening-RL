...
class Zelda_Env(gym.Env):
    def __init__(self, game_file, save_file, task_name=None, task_params=None, terminate_on_subtask=False):
       ...
       # 钥匙数量（背包内钥匙计数）
        ...
        self.explore_bonus = 1.0  # 探索新网格的正向奖励规模
 ...
 # 任务奖励权重配置（可被 task_params['reward_weights'] 覆盖）
    ...
    def get_distance(self):
        ...
        # 1. 如果是 'press_button' 任务
        if self.task_name == 'press_button' and hasattr(self, 'task_params') and self.task_params:
            area = self.task_params.get('area_xyxy')
            # 确保 area_xyxy (e.g., [80, 45, 84, 49]) 已在 task_params 中定义
        ...
        # 2. 如果是 'reach_area' 任务
        if self.task_name == 'reach_area' and hasattr(self, 'task_params') and self.task_params:
            target = self.task_params.get('target_xy')
            # 确保 target_xy (e.g., [82, 47]) 已在 task_params 中定义
